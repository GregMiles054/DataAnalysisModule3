...
WHERE o.status = 'paid'
GROUP BY o.order_id, c.customer_name, s.store_name, o.order_datetime, o.store_id
HAVING SUM(oi.quantity * p.price) >
(
    SELECT AVG(store_total)
    FROM (
        SELECT SUM(oi2.quantity * p2.price) AS store_total
        FROM orders o2
...
WITH daily_revenue AS (
    ...
    JOIN products p ON oi.product_id = p.product_id
    WHERE o.status = 'paid'
    GROUP BY o.store_id, s.store_name, DATE(o.order_datetime)
)
...
SELECT
    ...
    total_spend * 1.0 / SUM(total_spend) OVER () AS percent_of_total
...
WITH product_revenue AS (
    ...
    WHERE o.status = 'paid'
    GROUP BY o.store_id, s.store_name, p.product_name, cat.category_name
)
...
JOIN products p ON oi.product_id = p.product_id
WHERE o.status = 'paid'
GROUP BY o.store_id, s.store_name, o.payment_method;
...
SELECT
    ...
    revenue * 1.0 / SUM(revenue) OVER (PARTITION BY store_id) AS pct_of_store_revenue
...
WITH sales_totals AS (
    ...
    JOIN products p ON oi.product_id = p.product_id
    WHERE o.status = 'paid'
    GROUP BY o.store_id, p.product_id
)
...
SELECT
    ...
    i.on_hand,
    st.total_units_sold,
    (st.total_units_sold - i.on_hand) AS units_gap
FROM inventory i
JOIN sales_totals st
    ON i.store_id = st.store_id
   AND i.product_id = st.product_id
JOIN stores s ON i.store_id = s.store_id
JOIN products p ON i.product_id = p.product_id
WHERE i.on_hand < st.total_units_sold
